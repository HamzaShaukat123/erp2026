<div id="loader" style="background:#ffffff;">
    <div class="spinner-border text-primary" role="status">
        <span class="sr-only">Loading...</span>
    </div>
</div>

<header class="page-header" style="background:#f8f9fc; border-bottom:1px solid #e9ecef;">
	
	<div class="logo-container d-md-none">
		<a href="/" class="logo">
			<img src="/assets/img/logo.png" width="70px" alt="MFI Logo" />
		</a>

		<div id="userbox" class="userbox" style="float:right !important;">
			<a href="#" data-bs-toggle="dropdown" style="margin-right: 20px; color:#343a40;">
				<div class="profile-info"> 
					<span class="name" style="color:#212529; font-weight:600;">{{session('user_name')}}</span>
					<span class="role" style="color:#6c757d;">{{session('role_name')}}</span>
				</div>
				<i class="fa custom-caret" style="color:#495057;"></i>
			</a>

			<div class="dropdown-menu" style="border-radius:10px; overflow:hidden;">
				<ul class="list-unstyled m-0">

					<li>
						<a role="menuitem" tabindex="-1" href="#changePassword"
						   class="mb-1 mt-1 me-1 modal-with-zoom-anim ws-normal"
						   style="color:#0d6efd;">
						   <i class="bx bx-lock"></i> Change Password
						</a>
					</li>

					<li>	
						<form action="/logout" method="POST">
							@csrf
							<button type="submit"
								style="background: transparent; border: none; font-size: 14px; color:#dc3545; font-weight:500;">
								<i class="bx bx-power-off"></i> Logout
							</button>
						</form>
					</li>

					@if(session('user_role')==1 || session('user_role')==2)
					<li>
						<a href="{{ route('backup.database') }}"
						   style="color:#198754;">
						   <i class="bx bx-cloud-download"></i> DB Backup
						</a>
					</li>
					<li>
						<a href="{{ route('backup.files') }}"
						   style="color:#20c997;">
						   <i class="bx bx-files"></i> Files Backup
						</a>
					</li>
					@endif

				</ul>
			</div>

			<i class="fas fa-bars toggle-sidebar-left"
			   style="color:#343a40;"
			   data-toggle-class="sidebar-left-opened"
			   data-target="html"
			   data-fire-event="sidebar-left-opened"
			   aria-label="Toggle sidebar"></i>
		</div>

		<div class="right-wrapper text-end d-none d-md-block">
			<div id="userbox" class="userbox" style="float:right !important;">
				
				<a href="/pos"
				   class="btn btn-success"
				   style="background:linear-gradient(45deg,#28a745,#20c997); border:none; box-shadow:0 3px 8px rgba(0,0,0,0.15);">
				   POS System
				</a>

				<a href="#" data-bs-toggle="dropdown" style="margin-right: 20px; color:#343a40;">
					<div class="profile-info"> 
						<span class="name" style="color:#212529; font-weight:600;">{{session('user_name')}}</span>
						<span class="role" style="color:#6c757d;">{{session('role_name')}}</span>
					</div>
					<i class="fa custom-caret"></i>
				</a>

				<div class="dropdown-menu" style="border-radius:10px;">
					<ul class="list-unstyled m-0">

						<li>
							<a href="#changePassword"
							   class="mb-1 mt-1 me-1 modal-with-zoom-anim ws-normal"
							   style="color:#0d6efd;">
							   <i class="bx bx-lock"></i> Change Password
							</a>
						</li>

						<li>	
							<form action="/logout" method="POST">
								@csrf
								<button type="submit"
									style="background: transparent; border: none; font-size: 14px; color:#dc3545;">
									<i class="bx bx-power-off"></i> Logout
								</button>
							</form>
						</li>

					</ul>
				</div>

				<i class="fas fa-bars toggle-sidebar-left" style="color:#343a40;"></i>

			</div>
		</div>
	</div>

	<div class="logo-container d-none d-md-block">

		<div id="userbox" class="userbox" style="float:right !important;">

			<a class="btn btn-success"
			   href="/pos"
			   style="background:linear-gradient(45deg,#28a745,#20c997); border:none;">
			   POS System
			</a>

			<a href="#" data-bs-toggle="dropdown" style="margin-right: 20px; color:#343a40;">
				<div class="profile-info"> 
					<span class="name" style="font-weight:600; color:#212529;">{{session('user_name')}}</span>
					<span class="role" style="color:#6c757d;">{{session('role_name')}}</span>
				</div>
				<i class="fa custom-caret"></i>
			</a>

			<div class="dropdown-menu" style="border-radius:10px;">
				<ul class="list-unstyled m-0">

					<li>
						<a href="#changePassword" style="color:#0d6efd;">
							<i class="bx bx-lock"></i> Change Password
						</a>
					</li>

					<li>	
						<form action="/logout" method="POST">
							@csrf
							<button type="submit"
								style="background: transparent; border: none; font-size: 14px; color:#dc3545;">
								<i class="bx bx-power-off"></i> Logout
							</button>
						</form>
					</li>

					@if(session('user_role')==1 || session('user_role')==2)
					<li>
						<a href="{{ route('backup.database') }}" style="color:#198754;">
							<i class="bx bx-cloud-download"></i> DB Backup
						</a>
					</li>
					<li>
						<a href="{{ route('backup.files') }}" style="color:#20c997;">
							<i class="bx bx-file"></i> Files Backup
						</a>
					</li>
					@endif

				</ul>
			</div>

		</div>
	</div>

	<div id="changePassword" class="zoom-anim-dialog modal-block modal-block-danger mfp-hide">

		<form id="changePasswordForm"
			  method="post"
			  action="{{ route('change-user-password') }}"
			  style="width: 75%"
			  enctype="multipart/form-data"
			  onkeydown="return event.key != 'Enter';">

			@csrf

			<header class="card-header" style="background:#0d6efd; color:white;">
				<h2 class="card-title">Change Password</h2>
			</header>

			<div class="card-body">
				<div class="row form-group">    

					<div class="col-12 mb-2">
						<label>Current Password</label>
						<input type="password" class="form-control" style="border-color:#0d6efd;"
							   placeholder="Current Password"
							   id="current_password"
							   name="current_password"
							   required>
					</div> 

					<div class="col-12 mb-2">
						<label>New Password</label>
						<input type="password" class="form-control" style="border-color:#198754;"
							   placeholder="New Password"
							   id="new_password"
							   minlength="8"
							   name="new_password"
							   required>
					</div>

					<div class="col-12 mb-2">
						<label>Confirm New Password</label>
						<input type="password" class="form-control" style="border-color:#20c997;"
							   placeholder="Confirm New Password"
							   minlength="8"
							   id="confirm_new_password"
							   required>
					</div>

				</div>
			</div>

			<footer class="card-footer">
				<div class="row">
					<div class="col-md-12 text-end">

						<button type="submit"
								class="btn btn-primary"
								style="background:#0d6efd;">
							Change Password
						</button>

						<button type="button"
								class="btn btn-default modal-dismiss"
								style="background:#6c757d; color:white;">
							Cancel
						</button>

					</div>
				</div>
			</footer>

		</form>
	</div>

</header>