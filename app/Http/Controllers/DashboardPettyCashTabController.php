...
class DashboardPettyCashTabController extends Controller
{
    ...
    {
        $jv1 = petty_cash::where('id', $request->update_id)->first();

        if ($request->has('user_id_hidden') && $request->user_id_hidden) {
            $jv1->user_id = $request->user_id_hidden;
        }

        if ($request->has('update_add') || $request->update_add == 0) {
            $jv1->debit = $request->update_add;
        }

        if ($request->has('update_less') || $request->update_less == 0) {
        ...
        petty_cash::where('id', $request->update_id)->update([
            'user_id'    => $jv1->user_id,
            'debit'      => $jv1->debit,
            'credit'     => $jv1->credit,
            'date'       => $jv1->date,
