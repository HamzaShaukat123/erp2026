...
class HomeController extends Controller
{
    ...
    public function index()
    {
        if(session('user_role')==1 || session('user_role')==2){
            ...
            $activeUserId = session('id'); // OR auth()->id()
    
            return view('home', compact('receivables','payables','short_term_loan','long_term_loan','pdc','bal_pdc','banks','cash','foreign','login_users','last_month_purchase','last_month_sale','dash_pur_2_summary_monthly_companywise','coa','emply','activeUserId'));
        }
        else{
            $coa = AC::where('status', 1)->get();
            $emply = users::where('status', 1)->get();
